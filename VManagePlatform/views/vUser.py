...
def usermanage(request,id):
    if request.method == "GET":
        ...
        return render_to_response('vmUser/view_user.html',{"user":request.user,
                                                           "location":[{"name":"Home","url":'/'},
                                                                        {"name":"User Management","url":'/usermanage/?op=list'}],
        ...
        if op in ['active','superuser','delete','modify']:
            ...
            if op == 'active':
                ...
                try:
                    user.delete()
                    return  JsonResponse({"code":200,"data":None,"msg":"Operation Successful"})
                except:
        ...
        else:return  JsonResponse({"code":500,"data":None,"msg":"Unsupported Operation: You do not have permission to operate this item."})            
...
def user(request):
    if request.method == "GET":
        try:
            ...
            userList = []
        return render_to_response('vmUser/user_manage.html',{"user":request.user,"location":[{"name":"Home","url":'/'},{"name":"User Management","url":'/usermanage/?op=list'}],
                                            "userList":userList},
                              context_instance=RequestContext(request))
